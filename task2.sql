...
f.`name` as Flat,c.created as Start_Date,c.referral_contact as Referal_Contact,w.created as Last_Recharge,w.total as Wallet_Total
...
select case when transaction_type="Recharge" then "Purchases" else  transaction_type end from mcart_wallet;
...
select c.mccsid,c.name, w.total, from_unixtime(w.created) from mcart_customers c 
inner join mcart_wallet w on c.mccsid = w.mccsid 
